...
static jpeg_decode_out_t jpeg_decode_out_cb(JDEC *dec, void *bitmap, JRECT *rect)
{
    ...
    /* The stock callback copies one byte at a time.  With an RGB565 frame in
     * PSRAM that dominates the entire decode.  When decoder and requested
     * formats match, copy each MCU row as one contiguous block instead. */
    if (!cfg->flags.swap_color_bytes &&
        ((JD_FORMAT == 0 && cfg->out_format == JPEG_IMAGE_FORMAT_RGB888) ||
         (JD_FORMAT == 1 && cfg->out_format == JPEG_IMAGE_FORMAT_RGB565))) {
        const size_t row_bytes = (rect->right - rect->left + 1) * out_color_bytes;
        for (int y = rect->top; y <= rect->bottom; ++y) {
            memcpy(dst + ((size_t)y * line + rect->left) * out_color_bytes, in, row_bytes);
            in += row_bytes;
        }
        return 1;
    }
