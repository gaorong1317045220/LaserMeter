...
Project-authored software code uses MIT; project-authored documentation and artwork use CC BY-NC-SA 4.0. Third-party components retain their original licenses. The ONNX model is separate and keeps its embedded Ultralytics AGPL-3.0 declaration. `tools/package_release.ps1` produces the tested `Release/` directory, including the offset-0 merged firmware and portable PC app. The 2026-09-11 firmware build and release checks passed; Wi-Fi control that may touch Flash/NVS now runs on an internal-RAM stack instead of the PSRAM-backed PC link task, with request-ID completion checks and mutex/atomic protection for concurrent callers. See `BUILD_VERIFICATION.md`. Fresh-NVS hardware validation of the boot-loop fix and UI acceptance remain pending. Before publication, the owner still needs to confirm model/material rights, decide the default Wi-Fi credential policy, and add the hardware project link.
